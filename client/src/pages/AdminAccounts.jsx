...
export default function AdminAccounts() {
  ...
  const handleUpdateAdmin = async (updatedAdmin) => {
    updatedAdmin.user_id = updatedAdmin.user_id || selectedAdmin.user_id;
    ...
    try {
        ...
        const data = await res.json();
        if (data.error) {
            if (data.error === 'No changes made to the admin.') {
        ...
        // If changes were made, update the state as before
        const updatedAdminWithId = { ...updatedAdmin, user_id: data.userId || updatedAdmin.user_id };
        setAdmins(prevAdmins =>
            prevAdmins.map(admin => admin.user_id === updatedAdminWithId.user_id ? updatedAdminWithId : admin)
        );
        toast.success(data.message);
        setShowEditAccountModal(false);
    } catch (error) {
      ...
      <div className="relative overflow-y-auto max-h-[550px] shadow-md sm:rounded-lg mb-6 custom-scrollbar">
        {admins.length > 0 && (
          <table className="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
              {admins.map((admin, index) => (
                <tr key={admin.user_id || index} className="odd:bg-white even:bg-gray-50 border-b">
                  <th scope="row" className="px-6 py-4 font-medium text-gray-900">
                    {admin.username}
                  </th>
                  <td className="px-6 py-4">{admin.email}</td>
